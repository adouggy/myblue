...
public class MediaCache implements IUIDataCache {
	...
	public void initialData(
		...
		for (Contact c : list) {
			...
			mPhoto.put(c.getName(),
					new BitmapDrawable(ctx.getResources(), ContactADao.INSTANCE.getFacebookPhoto(c.getNumber())));
		}
		mMyPhoto = new BitmapDrawable(ctx.getResources(), ContactADao.INSTANCE.getFacebookPhoto("13601303722"));

		rotateAnimation_in = new RotateAnimation(0, 135, Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF, 0.5f);
		...
		rotateAnimation_out = new RotateAnimation(0, 315, Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF, 0.5f);
