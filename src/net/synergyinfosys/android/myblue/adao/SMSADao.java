...
public enum SMSADao {
	...
	public ArrayList<SMS> getSMS(String number) {
		ArrayList<SMS> list = new ArrayList<SMS>();
		// 读取收件箱中指定号码的短信，并且是已读
		Cursor cursor = this.mContext.getContentResolver().query(
				Uri.parse("content://sms/inbox"), 
				new String[] { "_id", "address", "body", "read", "date" }, 
				" address=? and read=? ",
				new String[] { number, 1 + "" }, 
				"date desc");
		if (cursor != null) {
	...
	public int deleteSMS( long id ){
		return this.mContext.getContentResolver().delete(Uri.parse("content://sms/" + id), null, null);
	}
